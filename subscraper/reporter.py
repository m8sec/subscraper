...
class SubReportQ(threading.Thread):
    ...
    def add(self, sub_entry):
        ...
        if sub_entry['Name'].lower() not in self.sub_cache and re.match(self.sub_regex, sub_entry['Name']):
...
class SubReporter(threading.Thread):
    ...
    def run(self):
        dns_lookup = False
        report_data = (
            f""""{self.sub_dict['Source']}","{self.sub_dict['Name']}","""
            if self.args.csv
            else self.sub_dict['Name']
        )
        ...
        if self.args.resolve or self.args.active:
            dns_lookup = self.sub_dict['DNS'] if 'DNS' in self.sub_dict.keys() else self.resolve_ip(
                self.sub_dict['Name'], self.args.timeout)

            # only output for -resolve
        if self.args.resolve:
            report_data += f'"{dns_lookup}",'
            tmp = f" [{', '.join(dns_lookup)}]"
        ...
        if self.args.cname:
            ...
            tmp = f" [{', '.join([x[:-1] if x.endswith('.') else x for x in cname])}]"
        ...
        if self.args.http:
            for port in self.args.http_port:
                ...
                if resp['code'] > 0:
                    report_data += f""""{resp['code']}","{resp['size']}","{resp['title']}","{resp['server']}","""

                    tmp = (
                        f" [{port}/{resp['proto'].upper()} - {resp['title'][:25]} ({resp['code']})"
                        + ("..]" if len(resp['title']) > 20 else "]")
                    )
