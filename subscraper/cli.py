...
def cmd_parser():
    ...
    brute.add_argument('-w', dest='wordlist',
                       default=path.join(path.dirname(path.realpath(__file__)), 'data', 'subdomains.txt'),
                       type=lambda x: file_exists(args, x, False), help='Custom wordlist for DNS brute force.')
    ...
    enum.add_argument('-r', '-resolve', dest='resolve', action='store_true',
                      help='Resolve IP address for each subdomain identified.')
    enum.add_argument('-c', '-cname', dest='cname', action='store_true',
                      help='Perform CNAME lookup for subdomain takeover checks')
    enum.add_argument('-http', action='store_true', help='Probe for active HTTP services.')
    enum.add_argument('-http-port', default='80,443', type=lambda x: delimiter2list(x),
                      help='HTTP ports to check, comma separated (80,443*)')
